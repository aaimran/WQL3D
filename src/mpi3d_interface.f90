...
contains
  ...
  subroutine find_neighbor3d(coord,cart_size,normal,comm,rank_neighbor)
    ...
    ! If no neighbor found, this might indicate non-adjacent blocks or single-block mode
    ! This should not happen in normal 2-block operation, but is handled gracefully
    if (rank_neighbor == -1) then
       ! Keep current behavior: use last gathered rank as fallback
       rank_neighbor = ranks(min(comm_size, 2*n(1)*n(2)))
    end if
